...
local function gpt_request(dataJSON, callback, callbackTable)
  ...
  if vim.fn.executable("curl") == 0 then
    vim.fn.confirm(
      "curl installation not found. Please install curl to use Backseat",
      "&OK",
      1,
      "Warning"
    )
...
local function parse_response(response, partNumberString, bufnr)
  ...
  for _, line in ipairs(lines) do
    if
      (string.sub(line, 1, 5) == "line=")
      or string.sub(line, 1, 6) == "lines="
    then
  ...
  for _, suggestion in ipairs(suggestions) do
    -- Get the line number
    local lineString =
      string.sub(suggestion, 6, string.find(suggestion, ":") - 1)
    ...
    -- Get the message
    local message = string.sub(
      suggestion,
      string.find(suggestion, ":") + 1,
      string.len(suggestion)
    )
    ...
    if callbackTable.startingRequestCount == 1 then
      print(
        "Sending "
          .. bufname
          .. " ("
          .. callbackTable.lineCount
          .. " lines) and waiting for response..."
      )
    else
      print(
        "Sending "
  ...
  for i = 1, numRequests do
    ...
    if get_language() ~= "" and get_language() ~= "english" then
      text = text
        .. "\nRespond only in "
        .. get_language()
        .. ", but keep the 'line=<num>:' part in english"
    end
